...
const copydir = async (src, dest) => {
	...
	for (const entry of entries) {
		...
		if (lstat.isDirectory()) {
			operations.push(
				copydir(path.resolve(src, entry), path.resolve(dest, entry)),
			);
		} else if (lstat.isFile()) {
			operations.push(
				fs.copyFile(path.resolve(src, entry), path.resolve(dest, entry)),
			);
		}
	}
...
const buildOptions = {
	entryPoints: [path.resolve(entrybase, 'main/main.ts')],
...
await copydir(
	path.resolve(entrybase, 'renderer'),
	path.resolve(outbase, 'renderer'),
);
